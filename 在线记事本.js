...
// 动态旋转彩线 Canvas 背景组件（HTML + JS）
const STAR_CANVAS_HTML = `
<canvas id="starCanvas" style="position:fixed; top:0; left:0; width:100vw; height:100vh; z-index:-1; pointer-events:none; display:block;"></canvas>
<script>
(function() {
...
<style>
body { font-family: sans-serif; background:#0a0a0a; color:#f0f0f0; padding:20px; margin:0; }
.card { background: rgba(255, 255, 255, 0.08); backdrop-filter: blur(16px); border: 1px solid rgba(255, 255, 255, 0.15); border-radius: 12px; padding: 20px; max-width: 600px; margin: 20px auto; }
h1 { color:#fff; margin-top: 0; }
ul { list-style:none; padding:0; }
li { margin:15px 0; border-bottom: 1px solid rgba(255, 255, 255, 0.1); padding-bottom: 10px; }
li:last-child { border-bottom: none; }
a { text-decoration:none; color:#80b3ff; font-size:1.1em; font-weight: bold; }
a:hover { text-decoration:underline; }
.time-info { display: flex; justify-content: space-between; font-size: 12px; color:#aaa; margin-top:4px;}
</style>
</head>
<body>
${STAR_CANVAS_HTML}
...
async function loadList(){
  try{
    ...
    arr.forEach(item => {
      ...
      li.innerHTML = icon + '<a href="/' + encodeURIComponent(item.name) + '">' + htmlEscape(item.name) + '</a>'
...
<style>
body { font-family: sans-serif; background:#0a0a0a; color:#fff; padding:20px; display:flex; justify-content:center; align-items:center; height:100vh; margin:0; }
.container { background: rgba(255, 255, 255, 0.08); backdrop-filter: blur(16px); border: 1px solid rgba(255, 255, 255, 0.15); padding:30px; border-radius:16px; text-align:center; box-shadow: 0 8px 32px rgba(0,0,0,0.3); }
input { padding:10px 14px; margin:15px 0; width:220px; background: rgba(0,0,0,0.5); border: 1px solid rgba(255,255,255,0.2); color:#fff; border-radius: 8px; outline:none; }
button { padding:10px 20px; cursor:pointer; background: #0071e3; border:none; color:#fff; border-radius: 8px; font-weight:bold; }
button:hover { background: #0077ed; }
.error { color:#ff6b6b; display:none; margin-bottom: 10px; font-size: 14px; }
</style>
</head>
<body>
${STAR_CANVAS_HTML}
...
.container{position:absolute;top:20px;right:20px;bottom:20px;left:20px;display:flex;flex-direction:column; background: rgba(255, 255, 255, 0.05); backdrop-filter: blur(16px); border: 1px solid rgba(255, 255, 255, 0.15); border-radius: 16px; padding: 20px; box-sizing: border-box;}
#content{flex:1;margin:0;padding:15px;overflow-y:auto;resize:none;width:100%;box-sizing:border-box;border:1px solid rgba(255,255,255,0.1);outline:none;font-size:1em; background: rgba(0, 0, 0, 0.3); color:#fff; border-radius: 8px;}
.controls{display:flex;justify-content:space-between;align-items:center;margin-top:12px;font-size:12px; color:#aaa;}
.actions{display:flex;gap:10px;align-items:center;}
#saveBtn{padding:6px 16px;font-size:12px; background:#0071e3; border:none; color:#fff; border-radius:6px; cursor:pointer; font-weight:bold;}
#saveBtn:hover{background:#0077ed;}
#status{margin-top:5px; font-size:12px; color:#6ee7b7;}
</style>
</head>
<body>
${STAR_CANVAS_HTML}
<div class="container">
<textarea id="content" placeholder="开始记录笔记...">${htmlEscape(content)}</textarea>
<div class="controls">
